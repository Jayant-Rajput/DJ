...
const ContestCards = ({ contest, isBookmarked }) => {
    ...
    return (
        <div 
            className={`relative rounded-2xl overflow-hidden transition-all duration-500 transform ${isHovered ? 'scale-105 shadow-xl shadow-blue-500/20' : 'shadow-lg'}`}
        ...
        >
            ...
            <div className=" opacity-70 bg-gray-800 p-6">
                ...
                <div className="flex justify-between items-start mb-3">
                    <div className={`px-3 py-1 rounded-full text-sm font-medium bg-gradient-to-r ${platformStyle.gradient} text-white`}>
                    ...
                    >
                        <svg className={`w-6 h-6 ${isBookmarked ? 'text-yellow-400 fill-current' : 'text-gray-400'}`} 
                ...
                {/* Contest title */}
                <h2 className="text-xl font-semibold text-white mb-3 line-clamp-2 h-14">{contest.title}</h2>
                
                {/* Status indicator */}
                <div className="flex items-center mb-3">
                    <div className={`w-3 h-3 rounded-full mr-2 ${isUpcoming ? 'bg-green-500 animate-pulse' : 'bg-gray-500'}`}></div>
                    <span className={`text-sm ${isUpcoming ? 'text-green-400' : 'text-gray-400'}`}>
                ...
                <div className={`absolute inset-0 bg-gradient-to-tr ${platformStyle.gradient} opacity-0 transition-opacity duration-300 ${isHovered ? 'opacity-10' : ''}`}></div>
            </div>
        </div>
    );
};
